...
public class LoginGUI extends JFrame {
	...
	public LoginGUI() {
		...
		btn_hastaLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (fld_hastaTC.getText().length() == 0 || fld_hastaPassw.getText().length() == 0) {
					Helper.showMsg("fill");
				} else {
					boolean key=true;
					try {
						...
						while (rs.next()) {
							if (fld_hastaTC.getText().equals(rs.getString("tcno"))
									&& fld_hastaPassw.getText().equals(rs.getString("password"))) {
					...
					if(key) {Helper.showMsg("Böyle Bir Hasta Bulunmamaktadýr. Lütfen Kayýt Olunuz !");}
				}
			}
		});
					...
					try {
						...
						while (rs.next()) {
							if (fld_doctorTC.getText().equals(rs.getString("tcno"))
									&& fld_doctorPassw.getText().equals(rs.getString("password"))) {
